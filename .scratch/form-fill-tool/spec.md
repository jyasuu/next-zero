...
- When `autoApplyWhenValid` is enabled, a valid verdict applies its values automatically (once per distinct values-set, guarded by a ref so re-renders do not re-apply). The auto-apply path calls `applyFormFill(toolId, values, { onlyIfEmpty: true })`, so it never clobbers fields the user has already filled — this also covers chat-history restore, where a stored valid verdict re-mounts `FormFillView` and would otherwise re-apply stale values over the form.
